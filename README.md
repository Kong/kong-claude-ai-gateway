...
| 2 | Key auth at Kong | [docs-2.0/02-key-auth-2.0.md](docs-2.0/02-key-auth-2.0.md) | ✅ config verified — key-auth is an `identity_providers` entry attached per-model via `access.identity_providers`, same mechanism module 3 uses for OIDC (corrected 2026-07-23; a `policies:`-attached `global: true` policy was a real dead end, but not the only path); re-verified via `kongctl diff` only as of 2026-07-23, not a mutating apply · ❌ traffic untested · ⚠️ credential secret value not retrievable after creation (beta API limitation) |
...
| 6 | Kong observability | [docs-2.0/06-observability-2.0.md](docs-2.0/06-observability-2.0.md) | ✅ dashboard created & confirmed live via the Konnect API — filtered by `ai_request_model`, not `route` (no route-equivalent exists for AI Gateway 2.0) · ⚠️ tiles unverified — no traffic sent (no `kong-2.0/06-*.yaml`, Admin/cloud-API-only, matching 1.x's module 6) |
...
├── kong-2.0/               # AI Gateway 2.0 track: one kongctl YAML per module, cumulative (except module 6, see its doc)
