# classic-kong-gateway

A test lab for running the Claude Desktop app against 3rd-party inference
routed through classic Kong Gateway (1.x, decK-based): Konnect (control
plane) + a local Kong Gateway data plane acting as the "Claude Gateway".
Walks through progressively advanced use cases — auth, per-user model
limits, rate limiting, observability, tracing, metering — each layered on
one long-running gateway.

**Superseded, kept for reference.** The actively maintained build of this
same idea is the AI Gateway 2.0 track at the repo root (`../README.md`) —
start there unless you specifically need the classic-gateway/decK
approach this folder documents.
...
├── certs/                  # data-plane client cert/key (git-ignored)
